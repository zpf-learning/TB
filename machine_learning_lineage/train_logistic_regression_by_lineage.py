...
BASE_OUTPUT_DIR = './tb_results_LR_lineage/'
...
def run_experiment_by_lineage():
    """Run lineage-stratified logistic-regression evaluation."""
    print("Loading data...")
    X_raw = pd.read_csv(X_FILE).set_index('uniqueid')
    Y_raw = pd.read_csv(Y_FILE).set_index('uniqueid')

    X_all_with_lin = X_raw[X_raw['lineage'].notna()].copy()
    all_lineages = X_all_with_lin['lineage'].unique()

    common_ids = X_all_with_lin.index.intersection(Y_raw.index)
    X_main, Y_main = X_all_with_lin.loc[common_ids], Y_raw.loc[common_ids]

    print(f"Valid total sample count: {len(X_main)} | lineage list: {list(all_lineages)}")

    for lin in all_lineages:
        print(f"\n" + "=" * 60)
        print(f">>> Processing lineage: {lin} (LR)")
        print("=" * 60)
        ...
        if len(X_lin) < 10:
            print(f"  [!] Lineage {lin} has insufficient total samples ({len(X_lin)}); skipping analysis.")
            continue
        ...
        for drug in DRUGS:
            ...
            print(f"  [+] {drug.ljust(4)} : evaluating (N={len(y_drug)}, R={counts.get(1, 0)})")
            ...
            for fold, (train_idx, test_idx) in enumerate(skf.split(X_drug, y_drug)):
                ...
                if fold == 0:
                    ...
                    print(
                        f"      Fold 0: Conv={c_status}({iters_done} iters) | TrainAcc={train_acc:.3f} | TestAcc={test_acc:.3f} | Gap={overfit_gap:.3f}")
                    ...
                    plt.title(f'{drug} Feature Importance (LR - {lin})')
                    plt.tight_layout()
            ...
            sns.heatmap(total_cm, annot=True, fmt='d', cmap='Blues', xticklabels=['S (0)', 'R (1)'],
                        yticklabels=['S (0)', 'R (1)'])
            plt.title(f'{drug} Total CM (LR - {lin})')
            plt.tight_layout()
            ...
            if fold_results:
                ...
                fold_df.to_csv(os.path.join(lin_output_dir, f'per_fold_LR_{lin}_{drug}.csv'), index=False)
                ...
                if all_who_fold_metrics:
                    ...
                    fold_who_df.to_csv(os.path.join(lin_output_dir, f'per_fold_WHO_LR_{lin}_{drug}.csv'), index=False)
        ...
        if all_drug_summaries:
            ...
            summary_df.to_csv(os.path.join(lin_output_dir, f'drug_performance_LR_{lin}.csv'), index=False)

            y_pred_matrix.to_csv(os.path.join(lin_output_dir, f'y_pred_matrix_LR_{lin}.csv'))
            y_prob_matrix.to_csv(os.path.join(lin_output_dir, f'y_prob_matrix_LR_{lin}.csv'))
            ...
            if has_curves:
                ...
                ax_roc.set_title(f'Combined ROC Curves (LR - {lin})', fontweight='bold')
                ax_roc.legend(loc="center left", bbox_to_anchor=(1.02, 0.5), borderaxespad=0.)
                ax_roc.grid(True, linestyle=':', alpha=0.7)
                fig_roc.tight_layout()
                fig_roc.savefig(os.path.join(lin_output_dir, f'Combined_ROC_Curves_LR_{lin}.png'), dpi=300,
                                bbox_inches='tight')
                ...
                ax_prc.set_title(f'Combined PR Curves (LR - {lin})', fontweight='bold')
                ...
                fig_prc.savefig(os.path.join(lin_output_dir, f'Combined_PRC_Curves_LR_{lin}.png'), dpi=300,
                                bbox_inches='tight')
            plt.close(fig_roc);
            plt.close(fig_prc)
            ...
            if all_who_fold_metrics_collector:
                ...
                who_summary.to_csv(os.path.join(lin_output_dir, f'WHO_layered_metrics_LR_{lin}.csv'), index=False)

        print(f"--- [Complete] Lineage {lin} analysis ---")

    print(f"\nAll lineage evaluations complete. Results saved in: {BASE_OUTPUT_DIR}")


if __name__ == "__main__":
    run_experiment_by_lineage()
