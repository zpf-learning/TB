...
BASE_OUTPUT_DIR = './tb_results_XGB_lineage/'
...
def run_xgb_experiment_by_lineage():
    """Run lineage-stratified XGBoost evaluation."""
    print("Loading data...")
    ...
    X_all, Y_all = X_with_lin.loc[common_ids], Y_raw.loc[common_ids]

    print(f"Valid total sample count: {len(X_all)} | lineage list: {list(all_lineages)}")

    for lin in all_lineages:
        print(f"\n" + "=" * 60)
        print(f">>> Processing lineage (XGBoost): {lin}")
        print("=" * 60)
        ...
        if len(X_lin) < 10:
            print(f"  [!] Lineage {lin} has insufficient samples; skipping.")
            continue
        ...
        for drug in DRUGS:
            ...
            if len(counts) < 2 or counts.get(1, 0) < 3:
                reason = "single class" if len(counts) < 2 else f"too few positive samples ({counts.get(1, 0)})"
                print(f"  [-] {drug.ljust(4)} : skipped ({reason})")
                continue

            print(f"  [+] {drug.ljust(4)} : training (N={len(y_drug)}, R={counts.get(1, 0)})")
            ...
            for fold, (train_idx, test_idx) in enumerate(skf.split(X_drug, y_drug)):
                X_train_val, X_test = X_drug.iloc[train_idx], X_drug.iloc[test_idx]
                y_train_val, y_test = y_drug.iloc[train_idx], y_drug.iloc[test_idx]

                if len(np.unique(y_train_val)) < 2:
                    print(f"    [!] Warning: one training fold for {drug} has insufficient classes; skipping this fold")
                    continue

                # Create an inner validation split so early stopping never observes the outer test fold.
                inner_n_folds = min(5, int(y_train_val.value_counts().min()))
                if inner_n_folds < 2:
                    print(f"    [!] Skipping fold {fold}: insufficient class counts for inner validation split")
                    continue
                inner_skf = StratifiedKFold(n_splits=inner_n_folds, shuffle=True, random_state=2026)
                inner_train_idx, inner_val_idx = list(inner_skf.split(X_train_val, y_train_val))[0]
                X_train, X_val = X_train_val.iloc[inner_train_idx], X_train_val.iloc[inner_val_idx]
                y_train, y_val = y_train_val.iloc[inner_train_idx], y_train_val.iloc[inner_val_idx]
                ...
                iters_done = model.best_iteration
                is_converged = iters_done < (model.n_estimators - 1)
                ...
                if fold == 0:
                    ...
                    print(
                        f"      Fold 0: Conv={c_status}({iters_done} trees) | TrainAcc={train_acc:.3f} | TestAcc={test_acc:.3f} | Gap={overfit_gap:.3f}")

                    plt.figure(figsize=(8, 6))
                    epochs_range = range(0, len(train_loss))
                    plt.plot(epochs_range, train_loss, label='Train Loss', color='blue', lw=2)
                    plt.plot(epochs_range, val_loss, label='Validation Loss', color='orange', lw=2)
                    plt.axvline(x=iters_done, color='red', linestyle='--', label=f'Best Iteration ({iters_done})')
                    plt.legend()
                    plt.ylabel('Log Loss', fontweight='bold')
                    plt.xlabel('Boosting Rounds (Trees)', fontweight='bold')
                    plt.title(f'{drug} XGBoost Train vs Val Loss ({lin})', fontweight='bold')
                    ...
                    top_20 = feat_imp.sort_values(by='Importance', ascending=False).head(20)
                    plt.figure(figsize=(10, 8))
                    sns.barplot(x='Importance', y='Feature', data=top_20, palette='magma', hue='Feature', legend=False)
                    plt.title(f'{drug} XGBoost Feature Importance ({lin})')
                    plt.tight_layout()
            ...
            sns.heatmap(total_cm, annot=True, fmt='d', cmap='Blues', xticklabels=['S (0)', 'R (1)'],
                        yticklabels=['S (0)', 'R (1)'])
            plt.title(f'{drug} Total CM (XGB - {lin})')
            plt.tight_layout()
            ...
            if fold_results:
                ...
                fold_df.to_csv(os.path.join(lin_output_dir, f'per_fold_XGB_{lin}_{drug}.csv'), index=False)
                ...
                if all_who_fold_metrics:
                    ...
                    fold_who_df.to_csv(os.path.join(lin_output_dir, f'per_fold_WHO_XGB_{lin}_{drug}.csv'), index=False)
        ...
        if all_drug_summaries:
            ...
            summary_df.to_csv(os.path.join(lin_output_dir, f'drug_performance_XGB_{lin}.csv'), index=False)

            y_pred_matrix.to_csv(os.path.join(lin_output_dir, f'y_pred_matrix_XGB_{lin}.csv'))
            y_prob_matrix.to_csv(os.path.join(lin_output_dir, f'y_prob_matrix_XGB_{lin}.csv'))
            ...
            if has_curves:
                ...
                ax_roc.set_title(f'Combined ROC Curves (XGBoost - {lin})', fontweight='bold')
                ax_roc.legend(loc="center left", bbox_to_anchor=(1.02, 0.5), borderaxespad=0.)
                ax_roc.grid(True, linestyle=':', alpha=0.7)
                fig_roc.tight_layout()
                fig_roc.savefig(os.path.join(lin_output_dir, f'Combined_ROC_Curves_XGB_{lin}.png'), dpi=300,
                                bbox_inches='tight')
                ...
                ax_prc.set_title(f'Combined PR Curves (XGBoost - {lin})', fontweight='bold')
                ...
                fig_prc.savefig(os.path.join(lin_output_dir, f'Combined_PRC_Curves_XGB_{lin}.png'), dpi=300,
                                bbox_inches='tight')
            plt.close(fig_roc);
            plt.close(fig_prc)
            ...
            if all_who_fold_metrics_collector:
                ...
                who_summary.to_csv(os.path.join(lin_output_dir, f'WHO_layered_metrics_XGB_{lin}.csv'), index=False)

        print(f"--- [Complete] Lineage {lin} XGBoost analysis ---")

    print(f"\nResults path: {BASE_OUTPUT_DIR}")


if __name__ == "__main__":
    run_xgb_experiment_by_lineage()
