...
def run_rf_experiment():
    """Run random-forest evaluation across target drugs."""
    print("Loading data...")
    X = pd.read_csv(X_FILE).set_index('uniqueid')
    if 'source_dataset' in X.columns:
        X = X.drop(columns=['source_dataset'])

    Y = pd.read_csv(Y_FILE).set_index('uniqueid')
    common_ids = X.index.intersection(Y.index)
    X, Y = X.loc[common_ids], Y.loc[common_ids]
    ...
    for drug in DRUGS:
        if drug not in Y.columns:
            continue

        y_drug_raw = Y[drug].dropna()
        y_drug = y_drug_raw.astype(int)
        X_drug = X.loc[y_drug.index]

        y_counts = y_drug.value_counts().to_dict()
        if len(y_counts) < 2 or y_counts.get(1, 0) < 5:
            print(f"--- Skipping drug: {drug.ljust(4)} | insufficient resistant samples: {y_counts} ---")
            continue

        print(f"--- Processing: {drug.ljust(4)} (RF) | sample distribution: {y_counts} ---")
        ...
        for fold, (train_idx, test_idx) in enumerate(skf.split(X_drug, y_drug)):
            ...
            if fold == 0:
                ...
                plt.title(f'{drug} RF Feature Importance (Fold 1)')
                plt.tight_layout()
        ...
        fold_df.to_csv(os.path.join(OUTPUT_DIR, f'per_fold_RF_{drug}.csv'), index=False)

        if all_who_fold_metrics:
            fold_who_df = pd.DataFrame(all_who_fold_metrics)
            fold_who_df.insert(0, 'Drug', drug)
            fold_who_df.to_csv(os.path.join(OUTPUT_DIR, f'per_fold_WHO_RF_{drug}.csv'), index=False)
    ...
    summary_df.to_csv(os.path.join(OUTPUT_DIR, 'drug_performance_summary_RF.csv'), index=False)

    y_pred_matrix.to_csv(os.path.join(OUTPUT_DIR, 'y_pred_matrix_RF.csv'))
    y_prob_matrix.to_csv(os.path.join(OUTPUT_DIR, 'y_prob_matrix_RF.csv'))
    ...
    ax_roc.set_title('Combined ROC Curves Across Drugs (RF)', fontweight='bold')
    ax_roc.legend(loc="center left", bbox_to_anchor=(1.02, 0.5), borderaxespad=0.)
    ax_roc.grid(True, linestyle=':', alpha=0.7)
    fig_roc.tight_layout()
    fig_roc.savefig(os.path.join(OUTPUT_DIR, 'Combined_ROC_Curves_RF.png'), dpi=300, bbox_inches='tight')
    plt.close(fig_roc)
    ...
    ax_prc.set_title('Combined Precision-Recall Curves Across Drugs (RF)', fontweight='bold')
    ...
    fig_prc.savefig(os.path.join(OUTPUT_DIR, 'Combined_PRC_Curves_RF.png'), dpi=300, bbox_inches='tight')
    plt.close(fig_prc)
    ...
    if all_who_fold_metrics:
        ...
        who_summary.to_csv(os.path.join(OUTPUT_DIR, 'WHO_layered_metrics_RF.csv'), index=False)

    print(f"\nRF experiment complete! Results saved in: {OUTPUT_DIR}")
