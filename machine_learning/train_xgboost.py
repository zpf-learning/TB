...
def main():
    ...
    if 'source_dataset' in X_all.columns:
        X_all = X_all.drop(columns=['source_dataset'])
    ...
    print(f"Valid total sample count: {len(X_all)}")
    ...
    for drug in DRUGS:
        ...
        if len(y_counts) < 2 or y_counts.get(1, 0) < 5:
            print(f"--- Skipping drug: {drug.ljust(4)} | insufficient sample distribution: {y_counts} ---")
            continue

        print(f"--- Processing: {drug.ljust(4)} (XGBoost) | sample counts: {y_counts} ---")
        ...
        for fold, (train_idx, test_idx) in enumerate(skf.split(X_drug, y_drug)):
            ...
            # Monitor validation loss on the inner split, leaving the outer test fold untouched.
            ...
            if fold == 0:
                ...
                print(
                    f"    Fold 0: Conv={conv_status}({iters_done} trees) | TrainAcc={train_acc:.3f} | TestAcc={test_acc:.3f} | Gap={overfit_gap:.3f}")

                plt.figure(figsize=(8, 6))
                epochs = len(train_loss)
                x_axis = range(0, epochs)
                plt.plot(x_axis, train_loss, label='Train Loss', color='blue', lw=2)
                plt.plot(x_axis, val_loss, label='Validation Loss', color='orange', lw=2)
                ...
                plt.title(f'{drug} XGBoost Train vs Validation Loss (Fold 1)', fontweight='bold')
                ...
                plt.title(f'{drug} XGBoost Feature Importance (Fold 1)')
                plt.tight_layout()
                plt.savefig(os.path.join(drug_plot_dir, 'importance.png'))
                plt.close()

            y_true_fold = Y_all.loc[y_test.index, DRUGS]
        ...
        sns.heatmap(total_cm, annot=True, fmt='d', cmap='Blues', xticklabels=['S (0)', 'R (1)'],
                    yticklabels=['S (0)', 'R (1)'])
        plt.title(f'{drug} Total Confusion Matrix (XGBoost)')
        plt.tight_layout()
        ...
        fold_df.to_csv(os.path.join(OUTPUT_DIR, f'per_fold_XGB_{drug}.csv'), index=False)

        if all_who_fold_metrics:
            fold_who_df = pd.DataFrame(all_who_fold_metrics)
            fold_who_df.insert(0, 'Drug', drug)
            fold_who_df.to_csv(os.path.join(OUTPUT_DIR, f'per_fold_WHO_XGB_{drug}.csv'), index=False)
        ...
        drug_avg['Drug'] = drug
        drug_avg['Count_S(0)'] = y_counts.get(0, 0)
        drug_avg['Count_R(1)'] = y_counts.get(1, 0)
    ...
    summary_df.to_csv(os.path.join(OUTPUT_DIR, 'drug_performance_summary_XGB.csv'), index=False)

    y_pred_matrix.to_csv(os.path.join(OUTPUT_DIR, 'y_pred_matrix_XGB.csv'))
    y_prob_matrix.to_csv(os.path.join(OUTPUT_DIR, 'y_prob_matrix_XGB.csv'))
    ...
    ax_roc.set_title('Combined ROC Curves Across Drugs (XGBoost)', fontweight='bold')
    ax_roc.legend(loc="center left", bbox_to_anchor=(1.02, 0.5), borderaxespad=0.)
    ax_roc.grid(True, linestyle=':', alpha=0.7)
    fig_roc.tight_layout()
    fig_roc.savefig(os.path.join(OUTPUT_DIR, 'Combined_ROC_Curves_XGB.png'), dpi=300, bbox_inches='tight')
    plt.close(fig_roc)
    ...
    ax_prc.set_title('Combined Precision-Recall Curves Across Drugs (XGBoost)', fontweight='bold')
    ...
    fig_prc.savefig(os.path.join(OUTPUT_DIR, 'Combined_PRC_Curves_XGB.png'), dpi=300, bbox_inches='tight')
    plt.close(fig_prc)
    ...
    if all_who_fold_metrics:
        ...
        who_summary.to_csv(os.path.join(OUTPUT_DIR, 'WHO_layered_metrics_XGB.csv'), index=False)
        print("WHO layered metrics calculation complete.")

    print(f"\nXGBoost experiment complete. Results saved in: {OUTPUT_DIR}")
